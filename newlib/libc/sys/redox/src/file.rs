...
libc_fn!(unsafe access(path: *mut c_char, _amode: c_int) -> c_int {
    // XXX amode
...
libc_fn!(unsafe _close(file: c_int) -> c_int {
    Ok(syscall::close(file as usize)? as c_int)
});

libc_fn!(unsafe dup(file: c_int) -> c_int {
    Ok(syscall::dup(file as usize, &[])? as c_int)
});

libc_fn!(unsafe dup2(file: c_int, newfile: c_int) -> c_int {
    Ok(syscall::dup2(file as usize, newfile as usize, &[])? as c_int)
});

libc_fn!(unsafe _fpath(file: c_int, buf: *mut c_char, len: c_int) -> c_int {
    let buf = slice::from_raw_parts_mut(buf as *mut u8, len as usize);
    Ok(syscall::fpath(file as usize, buf)? as c_int)
});

libc_fn!(unsafe _fstat(file: c_int, st: *mut syscall::Stat) -> c_int {
    Ok(syscall::fpath(file as usize, &mut *st)? as c_int)
});

libc_fn!(unsafe _fsync(file: c_int) -> c_int {
    Ok(syscall::fsync(file as usize)? as c_int)
});

libc_fn!(unsafe ftruncate(file: c_int, len: off_t) -> c_int {
    Ok(syscall::ftruncate(file as usize, len as usize)? as c_int)
});

libc_fn!(unsafe _lseek(file: c_int, ptr: c_int, dir: c_int) -> c_int {
    Ok(syscall::lseek(file as usize, ptr as isize, dir as usize)? as c_int)
});


libc_fn!(unsafe mkdir(path: *mut c_char, mode: mode_t) -> c_int {
    let path = CStr::from_ptr(path).to_string_lossy();
...
libc_fn!(unsafe _open(path: *mut c_char, flags: c_int, mode: mode_t) -> c_int {
    let path = CStr::from_ptr(path).to_string_lossy();
...
libc_fn!(unsafe pipe2(pipefd: &mut [c_int; 2], flags: c_int) -> c_int {
...
libc_fn!(unsafe _read(file: c_int, buf: *mut c_char, len: c_int) -> c_int {
    let buf = slice::from_raw_parts_mut(buf as *mut u8, len as usize);
    Ok(syscall::read(file as usize, buf)? as c_int)
});

libc_fn!(unsafe rmdir(path: *mut c_char) -> c_int {
    let path = CStr::from_ptr(path).to_string_lossy();
